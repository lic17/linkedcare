...
func JobReRun(namespace, jobName, resourceVersion string) error {
	...
	for i := 0; i < retryTimes; i++ {
		_, err = k8sClient.BatchV1().Jobs(namespace).Create(context.TODO(), &newJob, metav1.CreateOptions{})
		if err != nil {
...
func deleteJob(namespace, job string) error {
	...
	err := k8sClient.BatchV1().Jobs(namespace).Delete(context.TODO(), job, metav1.DeleteOptions{PropagationPolicy: &deletePolicy})
	return err
}
