...
func CreateKubeConfig(username string) error {
	...
	_, err := k8sClient.CoreV1().ConfigMaps(constants.LinkedcareControlNamespace).Get(context.TODO(), configName, metaV1.GetOptions{})
	...
	if errors.IsNotFound(err) {
		...
		_, err = k8sClient.CoreV1().ConfigMaps(constants.LinkedcareControlNamespace).Create(context.TODO(), &configMap, metaV1.CreateOptions{})
...
func GetKubeConfig(username string) (string, error) {
	k8sClient := client.ClientSets().K8s().Kubernetes()
	configName := fmt.Sprintf("kubeconfig-%s", username)
	configMap, err := k8sClient.CoreV1().ConfigMaps(constants.LinkedcareControlNamespace).Get(context.TODO(), configName, metaV1.GetOptions{})
	if err != nil {
...
func DelKubeConfig(username string) error {
	...
	err = k8sClient.CoreV1().ConfigMaps(constants.LinkedcareControlNamespace).Delete(context.TODO(), configName, metaV1.DeleteOptions{PropagationPolicy: &deletePolicy})
	if err != nil {
