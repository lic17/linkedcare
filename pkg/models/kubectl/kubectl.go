...
const (
	namespace = constants.LinkedcareControlNamespace
)

var DefaultImage = "linkedcare/kubectl:advanced-1.0.0"
...
func GetKubectlPod(username string) (models.PodInfo, error) {
	...
	deploy, err := k8sClient.AppsV1().Deployments(namespace).Get(context.TODO(), deployName, metav1.GetOptions{})
	if err != nil {
		klog.Errorln(err)
	...
	podList, err := k8sClient.CoreV1().Pods(namespace).List(context.TODO(), metav1.ListOptions{LabelSelector: labelSelector})
	if err != nil {
		klog.Errorln(err)
		return models.PodInfo{}, err
	}
...
func CreateKubectlDeploy(username string) error {
	k8sClient := client.ClientSets().K8s().Kubernetes()
	deployName := fmt.Sprintf("kubectl-%s", username)
	_, err := k8sClient.AppsV1().Deployments(namespace).Get(context.TODO(), deployName, metav1.GetOptions{})
	if err == nil {
		return nil
	}
		...
		Spec: appsv1.DeploymentSpec{
			...
			Template: v1.PodTemplateSpec{
				...
				Spec: v1.PodSpec{
					...
					ServiceAccountName: "linkedcare-cluster-admin",
				},
			},
		},
	}

	_, err = k8sClient.AppsV1().Deployments(namespace).Create(context.TODO(), &deployment, metav1.CreateOptions{})
...
func DelKubectlDeploy(username string) error {
	...
	err = k8sClient.AppsV1().Deployments(namespace).Delete(context.TODO(), deployName, metav1.DeleteOptions{PropagationPolicy: &deletePolicy})
	if err != nil {
		err := fmt.Errorf("delete username %s failed, reason:%v", username, err)
		return err
	}
