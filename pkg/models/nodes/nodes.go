...
func DrainNode(nodename string) (err error) {
	...
	_, err = k8sclient.CoreV1().Nodes().Patch(context.TODO(), nodename, types.StrategicMergePatchType, data, metav1.PatchOptions{})
	if err != nil {
		return err
	}
...
func drainEviction(nodename string, donech chan bool, errch chan error) {
	...
	podList, err := k8sclient.CoreV1().Pods("").List(context.TODO(), options)
	if err != nil {
		klog.Fatal(err)
		errch <- err
	}
	options.FieldSelector = ""
	daemonsetList, err := k8sclient.AppsV1().DaemonSets("").List(context.TODO(), options)
...
func evictPod(pod v1.Pod, GracePeriodSeconds int) error {
	...
	err := k8sclient.CoreV1().Pods(pod.Namespace).Evict(context.TODO(), &eviction)
	if err != nil {
		return err
	}
