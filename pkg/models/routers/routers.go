...
func GetAllRouters() ([]*corev1.Service, error) {
	...
	selector := labels.SelectorFromSet(labels.Set{"app": "linkedcare", "component": "ks-router", "tier": "backend"})
...
func createRouterService(namespace string, routerType corev1.ServiceType, annotations map[string]string) (*corev1.Service, error) {
	...
	service, err := k8sClient.CoreV1().Services(constants.IngressControllerNamespace).Create(context.TODO(), service, metav1.CreateOptions{})
	if err != nil {
		klog.Error(err)
		return nil, err
	}
...
func updateRouterService(namespace string, routerType corev1.ServiceType, annotations map[string]string) (*corev1.Service, error) {
	...
	service, err = k8sClient.CoreV1().Services(constants.IngressControllerNamespace).Update(context.TODO(), service, metav1.UpdateOptions{})
...
func deleteRouterService(namespace string) (*corev1.Service, error) {
	...
	err = k8sClient.CoreV1().Services(constants.IngressControllerNamespace).Delete(context.TODO(), serviceName, deleteOptions)
	if err != nil {
		klog.Error(err)
		return service, err
	}
...
func createOrUpdateRouterWorkload(namespace string, publishService bool, servicemeshEnabled bool) error {
	...
	k8sClient := client.ClientSets().K8s().Kubernetes()
	deployment, err := k8sClient.AppsV1().Deployments(constants.IngressControllerNamespace).Get(context.TODO(), deployName, metav1.GetOptions{})
	...
	if createDeployment {
		deployment, err = k8sClient.AppsV1().Deployments(constants.IngressControllerNamespace).Create(context.TODO(), deployment, metav1.CreateOptions{})
	} else {
		deployment, err = k8sClient.AppsV1().Deployments(constants.IngressControllerNamespace).Update(context.TODO(), deployment, metav1.UpdateOptions{})
	}
...
func deleteRouterWorkload(namespace string) error {
	...
	err := k8sClient.AppsV1().Deployments(constants.IngressControllerNamespace).Delete(context.TODO(), deploymentName, deleteOptions)
	if err != nil {
		klog.Error(err)
	}
	...
	for i := range replicaSets {
		err = k8sClient.AppsV1().ReplicaSets(constants.IngressControllerNamespace).Delete(context.TODO(), replicaSets[i].Name, deleteOptions)
		if err != nil {
			klog.Error(err)
		}
	}
